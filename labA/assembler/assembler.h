...
inline std::string RightTrim(std::string &s, const char *t = " \t\n\r\f\v") {
    ...
    ans=s.substr(0,cnt+1);
    return ans;
}

// trim from left & right
inline std::string Trim(std::string &s, const char *t = " \t\n\r\f\v") {
	std::string tmp=RightTrim(s, t);
	tmp=LeftTrim(tmp, t);
    return tmp;
}

inline int IsLC3Command(const std::string &str) {//return the type of command
    int index = 0;
...
inline int IsLC3TrapRoutine(const std::string &str) {//return the type of TRAP
    int index = 0;
...
inline int CharToDec(const char &ch) {//trans the hex char to dec
    if (ch >= '0' && ch <= '9') {
        return ch - '0';
    }
    if (ch >= 'A' && ch <= 'F') {
...
inline char DecToChar(const int &num) {//trans the dec to hex char 
    if (num <= 9) {
        return num + '0';
    } else {
        return num - 10 + 'A';
